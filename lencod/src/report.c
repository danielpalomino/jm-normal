...
void report_log(VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats) {
    ...
    if ((p_log = fopen("../filesCoderNormal/log.dat", "r")) == 0) // check if file exists
    {
        if ((p_log = fopen("../filesCoderNormal/log.dat", "a")) == NULL) // append new statistic at the end
        {
            snprintf(errortext, ET_SIZE, "Error open file %s  \n", "../filesCoderNormal/log.dat");
            error(errortext, 500);
        } else // Create header for new log file
        {
            fprintf(p_log, " ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ \n");
        ...
        fclose(p_log);
        if ((p_log = fopen("../filesCoderNormal/log.dat", "a")) == NULL) // File exists, just open for appending
        {
            snprintf(errortext, ET_SIZE, "Error open file %s  \n", "../filesCoderNormal/log.dat");
            error(errortext, 500);
        }
    ...
    p_log = fopen("../filesCoderNormal/data.txt", "a");
...
void report(VideoParameters *p_Vid, InputParameters *p_Inp, StatParameters *p_Stats) {
    ...
    if (p_Inp->Verbose != 0) {
        ...
        //Daniel - código para imprimir PSNR
        fprintf(resultsPSNRbits, "%7.3f,", snr->average[0]);
    ...
    //Daniel código para imprimir total bits
    fprintf(resultsPSNRbits, " %" FORMAT_OFF_T "\n", total_bits);
