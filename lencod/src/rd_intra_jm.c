...
*/
    ...
    for (i = block_y; i < BLOCK_SIZE + block_y; i++) {
        for (j = block_x; j < BLOCK_SIZE + block_x; j++) {
/*
            fprintf(predictedI4MB, "%d-(%d,%d)\t",currSlice->mb_pred[0][i][j],i,j);
*/
            predictedI4MBblock[i][j] = (int) currSlice->mb_pred[0][i][j];

        }
/*
        fprintf(predictedI4MB, "\n");
*/
    }
    ...
    if (indice == 15) {
        for (i = 0; i < 16; i++) {
            for (j = 0; j < 16; j++) {
                fprintf(predictedI4MB, "%d\t", predictedI4MBblock[i][j]);
            }
            fprintf(predictedI4MB, "\n");
        }
    }
...
int mode_decision_for_I16x16_MB_RDO(Macroblock* currMB, int lambda) {
    ...
    //BEGIN DANIEL
    for(i=currMB->opix_y; i < MB_BLOCK_SIZE + currMB->opix_y; i++) {
        for(j=currMB->pix_x; j < MB_BLOCK_SIZE + currMB->pix_x; j++) {
            fprintf(originalBlocks,"%d\t",currMB->p_Slice->p_Vid->pCurImg[i][j]);
        }
        fprintf(originalBlocks,"\n");
    }
    //END DANIEL
...
*/
        //DAniel Code
        distortionY = compute_SSD16x16_thres(currMB, PLANE_Y);
...
*/
    ...
    for (i = 0; i < MB_BLOCK_SIZE; i++) {
        for (j = 0; j < MB_BLOCK_SIZE; j++) {
            fprintf(predictedI16MB, "%d\t", currMB->p_Slice->mpr_16x16[PLANE_Y][best_mode][i][j]);
        }
        fprintf(predictedI16MB, "\n");
    }
